---
title: Deploying a Simple Webapp For Choosing A Drink Of The Month 
id: a1a1
subject: aws
...
## Problem
A webapp for choosing a drink of the month needs to be deployed to AWS while adhereing to the AWS Well-Architected Framework such as Operational excellence,  Security, Reliability, Performance efficiency, Cost optimization and Sustainability.

## Solution

We shall rely on Github Actions for CI/CD pipelines connected to ECR as a container registery while relying on ECS Fargate for serverless container deployment and RDS for managed database servers and Secrets Manager for automated secrets rotation and ALB for load balancing while setting up SGs and IAM Roles for secure service access and Private/Public Multi-AZ VPCs for high availabilty and NAT Gateways for Private subnet access.
